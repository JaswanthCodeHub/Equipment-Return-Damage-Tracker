...
class TrackerWorkflowTests(unittest.TestCase):
    ...
    def test_seed_user_name_is_rahul(self):
        response = self.login("user")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json()["user"]["name"], "Rahul")

    def test_google_sign_in_creates_user_account(self):
        response = self.client.post("/api/auth/google", json={"email": "rahul.google@example.com", "name": "Rahul Google"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json()["user"]["role"], "user")
        self.assertEqual(response.get_json()["user"]["name"], "Rahul Google")
        dashboard = self.client.get("/api/dashboard")
        self.assertEqual(dashboard.status_code, 200)
        self.assertEqual(dashboard.get_json()["role"], "user")
    ...
    def test_admin_can_save_notification_settings(self):
        self.login("admin")
        response = self.client.patch("/api/settings", json={
            "company_name": "SD Digitals Rentals",
            "support_email": "support@sd-digitals.com",
            "support_phone": "+91 99999 99999",
            "notify_booking_approvals": False,
            "notify_return_inspections": True,
            "notify_payment_refunds": False,
        ...
        self.assertEqual(data["company_name"], "SD Digitals Rentals")
        self.assertFalse(data["notify_booking_approvals"])
        self.assertFalse(data["notify_payment_refunds"])
    ...
    def test_complete_clean_return_workflow(self):
        ...
        self.logout(); self.login("user")
        settlement = self.client.get("/api/returns").get_json()[0]
        self.assertEqual(settlement["booking_id"], booking_id)
        self.assertEqual(settlement["booking_days"], 3)
        self.assertGreater(settlement["rental_amount"], 0)
        self.assertEqual(settlement["balance_refund"], settlement["deposit_amount"] - settlement["deposit_deduction"])
    ...
    def test_damage_claim_and_deduction_decision(self):
        ...
        self.assertEqual(decision.get_json()["status"], "closed")
        closed_return = self.client.get(f"/api/returns/{return_id}").get_json()
        self.assertEqual(closed_return["status"], "closed")
        self.logout(); self.login("user")
